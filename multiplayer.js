...
var Multiplayer = function (attached, callbacks, port, address) {
    'use strict';
    var	socket;
    ...
    if (typeof port === 'object') {
        ...
        var uri = address || (location.protocol + '//' + location.hostname) + ':' + (location.port || 4000);
        socket = io.connect(uri);
    }

    socket.on('connect', function () {
        debug.log('Connection established with Socket ID: ' + socket.id);
    });
